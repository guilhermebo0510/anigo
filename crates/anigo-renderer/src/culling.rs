...
impl SceneVolume {
    ...
    ///
    /// A caixa é a **circunscrita** (`half = radius`): ela precisa conter a
    /// esfera inteira, senão o teste de caixa poderia recusar geometria que a
    /// esfera aceitou (a caixa inscrita seria anti-conservadora).
    pub fn from_sphere(local: &BoundingSphere, world_matrix: &glam::Mat4) -> Self {
        let sphere = local.transformed(world_matrix);
        let half = sphere.radius;
        let aabb = Aabb {
