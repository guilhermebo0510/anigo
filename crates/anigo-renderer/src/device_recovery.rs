...
/// faz `send` em um canal `unbounded`. O dreno
/// ([`drain_to_diagnostics`]) acontece no ritmo do renderer — uma vez por
/// quadro, no `render_scene` — e é **não-bloqueante** (`try_iter`, nunca o
/// `iter` bloqueante do `mpsc`, que esperaria para sempre com o `sender` vivo).
pub struct UncapturedErrorBus {
    ...
    /// Registros aguardando dreno. O `Receiver` não expõe tamanho sem consumir,
    /// então o contador vive aqui: `pending()` é uma leitura **não-destrutiva**
    /// (inspecionar nunca esvazia o canal) e cada `send`/`dreno` o ajusta.
...
impl UncapturedErrorBus {
    ...
    fn note_drained(&self) {
        // Saturante por construção: cada registro drenado corresponde a um
        // `send` contabilizado, mas um `fetch_sub` cru poderia enrolar o
        // contador se algum produtor futuro não passar por `note_enqueued`.
        let _ = self.in_flight.fetch_update(Ordering::SeqCst, Ordering::SeqCst, |value| {
    ...
    pub fn install_on(&self, device: &wgpu::Device) {
        ...
        device.on_uncaptured_error(Box::new(move |error: wgpu::Error| {
            if sender
                .send(UncapturedErrorRecord::from_wgpu_error(&error))
                .is_ok()
            {
    ...
    /// Quantos erros ainda aguardam dreno (telemetria rápida, não-destrutiva:
    /// inspecionar o contador nunca consome o canal).
    ...
    ///
    /// Não-bloqueante: retorna imediatamente quando o canal está vazio (o
    /// `render_scene` chama isto uma vez por quadro — um dreno bloqueante
    /// congelaria o renderer).
