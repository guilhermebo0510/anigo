...
mod tests {
    ...
    fn hierarchy_validation_rejects_cycles_and_dangling_parents() {
        ...
        project.scene.nodes.push(
            NodeSlot::new(NodeId::from_slug("jacket"), "Jacket", NodeKind::Clothing)
                .child_of(root.clone()),
        );
        project.scene.nodes.push(
            NodeSlot::new(NodeId::from_slug("hood"), "Hood", NodeKind::Hair)
                .child_of(NodeId::from_slug("jacket")),
        );
        project.validate().expect("hierarchy is valid");
        assert_eq!(project.scene.children_of(Some(&root)).len(), 1);
        assert_eq!(project.scene.depth_of(&NodeId::from_slug("hood")), Some(2));

        // Pai inexistente.
        let mut dangling = project.clone();
        dangling.scene.nodes[1].parent_id = Some(NodeId::from_slug("missing"));
        assert!(matches!(
            dangling.validate(),
        ...
        cyclic.scene.nodes[0].parent_id = Some(NodeId::from_slug("hood"));
        assert!(matches!(
            cyclic.validate(),
            Err(ProjectError::InvalidHierarchy { .. })
        ));
