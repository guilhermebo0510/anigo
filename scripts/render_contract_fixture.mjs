...
export function renderContractFixture() {
  const contract = {
    ...
    passes: [
      {
        // Issue #14: a ordem de execução vem do `render_graph` abaixo; este
        // campo é o desempate declarado do passe dentro da biblioteca.
        order: 40,
        name: "outline",
        kind: "render",
      ...
      {
        // Issue #14: depth pre-pass. Roda o **mesmo** vertex shader do cel
        // (mesma transformação, mesmo modelo) sem fragment stage: o que ele
        // entrega é o z-buffer resolvido para o early-Z do passe cel.
        order: 0,
        name: "depth_prepass",
        kind: "render",
        shader: "cel_shading",
    ...
    // ---------------------------------------------------------------------
    // Issue #14: render graph. O DAG é a autoridade sobre ordem e ativação dos
    // passes; `anigo-renderer::render_graph` valida (ciclo/dependência
    // inexistente) e agenda, e o mesmo bloco é lido pelo viewport — é o que
    // garante paridade headless ⇄ viewport sem duas listas de ordem.
    //
    // `contract_pass: null` = nó já posicionado no DAG (com dependências e
    // recursos) cujo pipeline ainda não existe: ele fica desabilitado e não é
    // agendado, em vez de rodar como no-op disfarçado.
    // ---------------------------------------------------------------------
    render_graph: {
      module: "anigo-renderer::render_graph",
      scheduling: "kahn_topological",
      order_hint: "desempate determinístico (depois, pelo nome)",
      settings_from: "scene.render_graph (ativar/desativar e ordem por snapshot)",
      nodes: [
        {
          name: "sparse_morph",
          kind: "compute",
          contract_pass: "sparse_morph",
          depends_on: [],
          reads: ["base_vertices", "morph_deltas"],
          writes: ["morphed_vertices"],
          enabled: true,
          order_hint: -10,
          only_when: "gpu_morph_active",
          early_z_for: null,
        },
        {
          name: "depth_prepass",
          kind: "render",
          contract_pass: "depth_prepass",
          depends_on: ["sparse_morph"],
          reads: [],
          writes: ["depth"],
          enabled: true,
          order_hint: 0,
          early_z_for: "opaque_cel",
        },
        {
          name: "face_shadow_sdf",
          kind: "render",
          contract_pass: null,
          depends_on: ["depth_prepass"],
          reads: ["depth"],
          writes: ["face_sdf_mask"],
          enabled: false,
          order_hint: 10,
          early_z_for: null,
        },
        {
          ...
          kind: "render",
          contract_pass: "cel",
          depends_on: ["depth_prepass", "face_shadow_sdf"],
          reads: ["depth", "face_sdf_mask"],
          writes: ["color"],
          enabled: true,
          order_hint: 20,
          early_z_for: null,
        },
        {
          ...
          kind: "render",
          contract_pass: null,
          depends_on: ["opaque_cel"],
          reads: ["depth"],
          writes: ["color", "oit_accum"],
          enabled: false,
          order_hint: 30,
          early_z_for: null,
        },
        {
          name: "inverted_hull_outline",
          kind: "render",
          contract_pass: "outline",
          depends_on: ["opaque_cel", "hair_and_cloth"],
          reads: ["depth", "color"],
          writes: ["color"],
          enabled: true,
          order_hint: 40,
        ...
        },
        {
          ...
          kind: "render",
          contract_pass: null,
          depends_on: ["inverted_hull_outline"],
          reads: ["color", "depth"],
          writes: ["bloom_a"],
          enabled: false,
          order_hint: 50,
      ...
      resources: [
        {
          ...
          size: "viewport",
          samples: 4,
          first_use: "opaque_cel",
          last_use: "post_process",
        },
        {
          name: "depth",
          kind: "depth_stencil",
        ...
        },
        {
          ...
          size: "viewport",
          samples: 1,
          first_use: "face_shadow_sdf",
          last_use: "opaque_cel",
        },
        {
          // Issue #14 (economia de VRAM): o acumulador do OIT de cabelo/roupa e
          // o primeiro alvo da cadeia de bloom têm a **mesma chave** e janelas
          // de vida disjuntas — dividem uma alocação.
          name: "oit_accum",
          kind: "transient_color",
          format: "rgba16float",
          size: "viewport",
          samples: 1,
          first_use: "hair_and_cloth",
          last_use: "hair_and_cloth",
        },
        {
          name: "bloom_a",
          kind: "transient_color",
      ...
      aliasing: {
        strategy: "liveness_intervals",
        sharing_key: ["kind", "format", "size", "samples"],
        note:
          "Dois recursos com a mesma chave e janelas de vida disjuntas dividem " +
          "a alocação; com os passes de cabelo/pós-processamento desabilitados " +
          "o plano não tem nada para compartilhar (e reporta 0 bytes).",
        groups: [
          { name: "oit_and_bloom", members: ["oit_accum", "bloom_a"] },
        ],
      },
    },
    // ---------------------------------------------------------------------
